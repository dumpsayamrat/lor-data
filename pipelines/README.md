...
| `pipelines/`       | A Python directory that contains code for your Dagster repository                 |
| `pipelines_tests/` | A Python directory that contains tests for `pipelines`                      |
...
pyenv virtualenv $PYTHON_VERSION pipelines
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name pipelines python=PYTHON_VERSION
...
for powershell run to acticate virenv
```
...
Tests can be found in `pipelines_tests` and are run with the following command:

```bash
pytest pipelines_tests
```

As you create Dagster ops and graphs, add tests in `pipelines_tests/` to check that your
